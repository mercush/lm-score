# Setup
To run the benchmark, make sure you have installed uv and have activated the virtual environment. Then, 
run `cp .env.example .env` and then plug in the appropriate environment variables to configure the judge LM. See the Configuration section in README.md to see how to set the ENSEMBLE, AGGREGATION, and THINKING parameters.
```
time uv run tests/benchmark_lm_score.py
```
# Experimental design
We use the 30 rows from the company.db database as a benchmark. The database has 3 tables "EMAILS", "INVOICES", and "SALES_LEADS". Each table has 10 rows. Each set of 5 question is asked a different question.
...
The LM-judge and I made our labels based on if the LM outputs were _reasonable_ or _unreasonable_.
